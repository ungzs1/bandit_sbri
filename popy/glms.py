...
from popy.neural_data_tools import remove_trunctuated_neurons, add_firing_rates, downsample_time, time_normalize_session, build_trial_dataset, merge_behavior
...
class SingleUnitAnalysis:
    ...
    def load_data_for_glm(self):
        ...
        if not self.value_type is None:
            session_data = add_foraging_value(session_data, digitize=False)
        ...
        neural_data = remove_trunctuated_neurons(neural_data, mode='set_nan', delay_limit=20, print_usr_msg=False)  # set off-recording neurons to nan
